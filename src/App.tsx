...
function AppContent() {
  ...
  function renderContent() {
    if (user?.role === 'employee') {
      switch (activeView) {
        case 'my-expenses': return <ExpensesList onAddExpense={() => setUploadOpen(true)} refreshKey={refreshKey} />;
  ...
  return (
    <AppLayout>
      ...
      {user?.role === 'employee' && (
        <>
          <UploadExpenseModal open={uploadOpen} onClose={() => setUploadOpen(false)} onSuccess={handleExpenseCreated} />
          {activeView !== 'settings' && (
            <button
