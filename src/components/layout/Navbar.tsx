...
export function Navbar({ onMenuToggle, sidebarOpen }: NavbarProps) {
  const { t, theme, toggleTheme, toggleLanguage, language, user, logout, setActiveView } = useApp();
  ...
  return (
    <header className="sticky top-0 z-50 h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 flex items-center justify-between px-4 md:px-6">
      ...
      <div className="flex items-center gap-1 md:gap-2">
        <Button variant="ghost" size="icon" onClick={toggleLanguage} className="relative" aria-label="Toggle language">
          ...
          <DropdownMenuContent align="end" className="w-72">
            ...
            {flagged.slice(0, 5).map(e => (
              <DropdownMenuItem
                ...
                onClick={() => setActiveView(user?.role === 'admin' ? 'manual-review' : 'my-expenses')}
              >
                <span className="text-sm font-medium">{e.user?.name ?? e.merchant}</span>
                <span className="text-xs text-amber-600 dark:text-amber-400">
                  {e.merchant} · {e.flagReason ?? t('flagged')} · {e.currency} {e.amount.toFixed(2)}
                </span>
              </DropdownMenuItem>
            ))}
            {flagged.length === 0 && (
              <div className="px-3 py-4 text-sm text-slate-400 text-center">All caught up!</div>
            )}
          </DropdownMenuContent>
        </DropdownMenu>
